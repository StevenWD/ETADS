...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, head_count, model_dim, dropout=0.1,
                 max_relative_positions=0, with_saliency_selection=False, with_focus_attention=False):
        ...
        if with_saliency_selection:
            self.linear_selection_query = nn.Linear(model_dim, head_count * self.dim_per_head)
            self.linear_selection_key = nn.Linear(model_dim, head_count * self.dim_per_head)
        ...
        if with_focus_attention:
            ...
            uz = torch.randn(head_count, 1, self.dim_per_head)
            self.uz = Variable(uz, requires_grad=True).cuda()
            torch.nn.init.xavier_uniform_(self.uz)


        self.with_saliency_selection = with_saliency_selection
